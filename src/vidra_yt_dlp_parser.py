...
def options_parser(
    options: VidraOptions, force_recreate_cookie_file: bool = False
) -> list[str]:
    ...
    assert (
        ...
        and "cookies_from_webview" in options
    ...
    cookies_from_webview = options["cookies_from_webview"]
    ...
    if js_runtimes:
        for runtime, path in js_runtimes.items():
            comando.extend(["--js-runtimes", f"{runtime}:{path}"])
    # Network Options
    if proxy:
    ...
    if cookies is False and cookies_from_webview is False:
        comando.append("--no-cookies")
    elif paths.get("temp"):
        # Las cookies serán mezcladas y copiadas a un archivo temporal
        new_cookie_file = os.path.join(paths["temp"], "merged_cookies.txt")
        if force_recreate_cookie_file or not os.path.exists(new_cookie_file):
            with open(new_cookie_file, "w", encoding="utf-8") as f:
                f.write("# Netscape HTTP Cookie File\n")
                f.write("# This file was generated by Vidra! Do not edit.\n")
                if cookies and os.path.exists(cookies) and os.path.isfile(cookies):
                    with open(cookies, "r", encoding="utf-8") as c:
                        f.write(c.read())
                if (
                    cookies_from_webview
                    and os.path.exists(cookies_from_webview)
                    and os.path.isdir(cookies_from_webview)
                ):
                    for cookie_file in os.listdir(cookies_from_webview):
                        if not cookie_file.endswith(".txt"):
                            continue
                        cookie_path = os.path.join(cookies_from_webview, cookie_file)
                        if os.path.isfile(cookie_path):
                            with open(cookie_path, "r", encoding="utf-8") as c:
                                f.write(c.read())
        comando.extend(["--cookies", new_cookie_file])
    elif cookies_from_webview:
        # de haber cookies y cookies_from_webview, se priorizan las cookies de webview
        comando.extend(["--cookies", cookies_from_webview])
