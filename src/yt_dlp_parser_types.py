...
class VidraOptions(TypedDict):
    ...
    audio: NotRequired[Literal["default", T_AUDIO_OPTIONS]]
    # Presets para descargar la mejor pista de video disponible en ciertas resoluciones.
    video: NotRequired[Literal["default", T_VIDEO_OPTIONS]]
    # Presets para descargar subtítulos en ciertos idiomas.
    subtitles: NotRequired[Literal["default", T_SUBTITLE_OPTIONS]]
...
def is_valid_options(options: Any) -> TypeGuard[VidraOptions]:
    ...
    for key, value in options.items():
        if key == "audio":
            ...
            if (
                not (isinstance(value, str) and value in SUBTITLE_OPTIONS)
                and value != "default"
            ):
                return False
